%% Energy Cummunity Social welfare schedulling program (c) 2023
% main program includig financial indicators
...
global T epsilon2 epsilon3 lambda1 lambda2_max lambda3_max Smax...
       ...
       if isempty(reply)
        ...
        reply2 = input('Congestion considered? Y/N [N]:','s');
        if isempty(reply2)
           reply2 = 'N';
        end
Sbase=10;%MVA
...
%% WHOLESALE SPOT MARKET
 ...
 1133.00 1101.32 1244.88 1333.66 1353.47 1359.74 1358.42 1314.52 1148.51];% pu XM data
...
elasticity2_avg=-inv(epsilon2*PD2_0max/mean(lambda1));
elasticity3_avg=-inv(epsilon3*PD3_0max/mean(lambda1));
...
DemandCost2=sum(lambda2.*PD2);
DemandCost3=sum(lambda3.*PD3);
...
%Base no CE
PD2b=PD2max-inv(epsilon2)*lambda1;
PD3b=PD3max-inv(epsilon3)*lambda1;
DemandCost2b=sum(lambda1.*PD2b);
DemandCost3b=sum(lambda1.*PD3b);
PBASE=109830;%OPEX_NOEC USD/yr
Io=133000000;% CAPEX USD
S=365*(PBASE-PSP1);%Savings USD/yr
Rate=0.05;%Discount rate %
CashFlow=ones(1,20)*S;
NPV = -Io+pvfix(Rate,20,S);
...
fprintf('Demand 2 Energy bougth                Eur/day %6.2f \n',DemandCost2)
fprintf('Demand 3 Energy bougth                Eur/day %6.2f \n',DemandCost3)
fprintf('Demand 2+3 Energy bougth                Eur/day %6.2f \n',DemandCost)
...
fprintf('Energy Consumed by the demand 2     MWh/day %6.2f \n',-(ED2)*Sbase)
fprintf('Energy Consumed by the demand 3     MWh/day %6.2f \n',-(ED3)*Sbase)
fprintf('Energy Consumed by the demand 2+3   MWh/day %6.2f \n',-(ED2+ED3)*Sbase)
